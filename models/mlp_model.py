...
class MLP:
    ...
    def plot_decision_boundary(self, target_train, input_test, target_test, points=True, h=0.1):
        ...
        if points:
            plt.scatter(self.input[0, :], self.input[1, :], c=target_train, cmap=clr.ListedColormap(['red', 'green']),  label= 'Train')
            plt.scatter(input_test[0, :], input_test[1, :], c=target_test, cmap=clr.ListedColormap(['black', 'blue']), label = 'Test')
